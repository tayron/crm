...
public class GrupoBean implements IGrupoBean {
	...
	public void cadastrarDadosDoGrupo() {
		...
		try {
			...
			mensagem.exibirSucesso(null, TipoMensagem.CADASTRO_SUCESSO.getDescricao());
			grupo = null;
		} catch (ExcecaoServico e) {			
			mensagem.exibirErro(null, TipoMensagem.CADASTRO_ERRO.getDescricao());
		}
	}
	...
	public String exibirTelaAlterarGrupo(String id) {
		if(buscarDadosGrupo(id)){
			...
			Mensagem mensagem = new Mensagem();
			mensagem.exibirAviso(null, "Não foi possível recuperar os dados do grupo");
			return "index";
		}
	}	
	...
	public void alterarDadosDoGrupo() {
		...
		try {
			...
			mensagem.exibirSucesso(null, TipoMensagem.SALVO_SUCESSO.getDescricao());
		} catch (ExcecaoServico e) {			
			mensagem.exibirErro(null, TipoMensagem.SALVO_ERRO.getDescricao());
		}
	}
	...
	public void excluirGrupo(Grupo grupo){
		...
		try {
			...
			mensagem.exibirSucesso(null, TipoMensagem.EXCLUIDO_SUCESSO.getDescricao());
		} catch (ExcecaoServico e) {			
			mensagem.exibirErro(null, TipoMensagem.EXCLUIDO_ERRO.getDescricao());
		}		
	}
